export const fetchSeachNews = (name, seachDate) => {
  ...
  if (seachDate) {
    return fetch(
      `https://api.nytimes.com/svc/search/v2/articlesearch.json?fq=${name}&api-key=SDNZee2lZmjshzcMGajKH4vNi11jfCZr&page=0&begin_date=20221125&end_date=${todayDate}`
    ).then(response => {
      if (!response.ok) {
  ...
  return fetch(
    `https://api.nytimes.com/svc/search/v2/articlesearch.json?q=${name}&api-key=SDNZee2lZmjshzcMGajKH4vNi11jfCZr&facet_fields=section_name&facet_filter=true&begin_date=20150101`
  ).then(response => {
    if (!response.ok) {
      throw new Error(response.status);
    }
